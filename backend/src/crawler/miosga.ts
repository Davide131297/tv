...
// Extrahiere Gäste aus einer Caren Miosga Episode
async function extractGuestsFromEpisode(
  page: Page,
  episodeUrl: string
): Promise<GuestWithRole[]> {
  console.log(`🎬 Crawle Caren Miosga Episode: ${episodeUrl}`);
  ...
  // Warte auf Seiten-Content
  await page
    .waitForSelector("main, .content, article", { timeout: 15000 })
    .catch(() => {});
  ...
  // Primär: Suche nach der Episoden-Beschreibung
  let guestsWithRoles: GuestWithRole[] = await page
    .evaluate(() => {
      const guests: GuestWithRole[] = [];

      // Suche nach Beschreibungstext der Episode
      const descriptions = document.querySelectorAll(
        'p, .description, [class*="description"], [class*="summary"]'
      );

      for (const desc of descriptions) {
        const text = desc.textContent || "";

        // Suche nach "Caren Miosga mit" Pattern aus dem HTML
        if (text.includes("Caren Miosga")) {
          console.log("Gefundener Beschreibungstext:", text);

          // Extrahiere Namen nach "Caren Miosga mit" oder "Caren Miosga spricht mit"
          const patterns = [
            /Caren Miosga (?:mit|spricht mit)\s+(.+?)(?:\.|$)/i,
            /Caren Miosga.*?mit\s+(.+?)(?:\.|$)/i,
          ];
          ...
          for (const pattern of patterns) {
            ...
            if (match) {
              ...
              for (const part of guestParts) {
                // Extrahiere Namen und mögliche Rollen/Beschreibungen
                const cleanPart = part.replace(/^(dem|der|die)\s+/i, "").trim();

                // Suche nach Namen mit möglichen Beschreibungen in Klammern oder nach Komma
                const nameMatch = cleanPart.match(
                  /^([A-ZÄÖÜ][a-zäöü\-]+\s+[A-ZÄÖÜ][a-zäöü\-]+(?:\s+[A-ZÄÖÜ][a-zäöü\-]+)*)/
                );
                if (nameMatch) {
                  const name = nameMatch[1].trim();

                  // Suche nach Rolle/Beschreibung
                  const restText = cleanPart
                    .substring(nameMatch[0].length)
                    .trim();
                  const role =
                    restText
                      .replace(/^[,\(\)]\s*/, "")
  ...
  console.log("📋 Gäste aus Beschreibung:", guestsWithRoles);

  // Fallback: Suche im Titel oder anderen Elementen
  if (!guestsWithRoles.length) {
    console.log("🔄 Fallback: Suche im Titel...");

    guestsWithRoles = await page
      .evaluate(() => {
        const guests: GuestWithRole[] = [];

        // Suche im Titel nach "mit [Name]" Pattern
        ...
        for (const title of titles) {
          ...
          // Pattern für "... mit [Namen]"
          const match = text.match(
            /mit\s+([A-ZÄÖÜ][a-zäöü\-]+\s+[A-ZÄÖÜ][a-zäöü\-]+)/
          );
          if (match) {
            const name = match[1].trim();
            guests.push({ name, role: undefined });
            break;
          }
        }
  ...
  // Fallback: Alt-Text von Bildern
  if (!guestsWithRoles.length) {
    console.log("🔄 Fallback: Suche in Bild Alt-Texten...");

    const alt = await page
      .$eval(
        'img[alt*="Caren Miosga"], img[alt*="Miosga"]',
        (el) => el.getAttribute("alt") || ""
      )
      .catch(() => "");

    if (alt && alt.includes("mit")) {
      const match = alt.match(/mit\s+([A-ZÄÖÜ][^,\.:]+)/);
      if (match) {
        const name = match[1].trim();
        if (name.includes(" ") && name.length > 5) {
          guestsWithRoles.push({ name, role: undefined });
        }
      }
    }
  }
...
export async function crawlNewCarenMiosgaEpisodes(): Promise<void> {
...
if (require.main === module) {
  ...
  if (mode === "full" || mode === "all" || mode === "complete") {
    crawlAllCarenMiosgaEpisodes()
      ...
      .catch((error) => {
        console.error("❌ Vollständiger Caren Miosga Crawler Fehler:", error);
        process.exit(1);
      });
  } else {
    crawlNewCarenMiosgaEpisodes()
      .then(() => {
        console.log("✅ Inkrementeller Caren Miosga Crawler beendet");
        process.exit(0);
      })
      .catch((error) => {
        console.error("❌ Inkrementeller Caren Miosga Crawler Fehler:", error);
        process.exit(1);
      });
  }
}
