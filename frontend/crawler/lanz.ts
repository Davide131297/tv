...
export default async function CrawlLanz() {
  ...
  try {
    ...
    for (const episode of finalResults) {
      ...
      // (wird unten nach Batch-Analyse gespeichert)
    }

    // Batch AI-Call für politische Themenbereiche aller Episoden
    const lanzBatchInputs: BatchEpisodeInput[] = finalResults
      .filter((ep: EpisodeResult) => ep.date && ep.description)
      .map((ep: EpisodeResult, i: number) => ({
        index: i,
        description: ep.description!,
      }));

    const lanzBatchAreas = await getBatchPoliticalAreas(lanzBatchInputs);

    // Speichere Batch-Ergebnisse
    let batchIdx = 0;
    for (const episode of finalResults) {
      if (!episode.date || !episode.description) continue;
      const areaIds = lanzBatchAreas.get(batchIdx);
      batchIdx++;
      if (areaIds && areaIds.length > 0) {
        ...
        console.log(
          `${episode.date}: ${insertedAreas}/${areaIds.length} Themenbereiche eingefügt`,
        );
      }
    }
